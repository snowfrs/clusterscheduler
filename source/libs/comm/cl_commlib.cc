   ...
   if (ret_val == CL_RETVAL_OK) {
      ...
      switch (cl_com_create_threads) {
         ...
         case CL_RW_THREAD: {
            ...
            if (ret_val != CL_RETVAL_OK) {
               CL_LOG_STR(CL_LOG_ERROR, "error shutting down handle application read condition",
      ...
      while (fd) {
         ...
         switch (mode) {
            ...
            case CL_W_SELECT: {
               read &= false;
               write &= true;
               if (!write && fd->data->ready_for_writing)
...
static int cl_com_trigger(cl_com_handle_t *handle, int synchron) {
   ...
   /* do virtual select */
   if (synchron == 1) {
      retval = cl_com_open_connection_request_handler(&poll_handle, handle, handle->select_sec_timeout,
                                                      handle->select_usec_timeout, CL_RW_SELECT);
   } else {
   ...
   if (connection->data_flow_type == CL_CM_CT_STREAM) {
      ...
      if (is_debug_client) {
         ...
         while (connection->data_read_buffer_pos > 0) {
            ...
            for (unsigned long pos = 0; pos < connection->data_read_buffer_pos; pos++) {
            ...
            if (unparsed_string_start != -1) {
               ...
               for (int pos = unparsed_string_start; pos < (int)connection->data_read_buffer_pos; pos++) {
      ...
      */
      ...
      if (message->message_state == CL_MS_RCV_MIH) {
         ...
         connection->statistic->real_bytes_received =
                 connection->statistic->real_bytes_received + connection->data_read_buffer_pos;
         return_value = cl_xml_parse_MIH(&(connection->data_read_buffer[(connection->data_read_buffer_processed)]),
                                         connection->read_gmsh_header->dl, &mih_message);
      ...
      if (message->message_state == CL_MS_RCV) {
         ...
         connection->statistic->real_bytes_received =
                 connection->statistic->real_bytes_received + message->message_length;
               ...
               if (connection->connection_state == CL_CONNECTED) {
                  if (connection->connection_sub_state == CL_COM_WORK) {
                     ...
                     if (connection->was_accepted) {
                        ...
                        CL_LOG(CL_LOG_INFO,
                               "already sending a CCM to client, setting sub state to CL_COM_RECEIVED_CCM, client wants also to disconnect ...");
                        connection->connection_sub_state = CL_COM_RECEIVED_CCM;
                     }
                     ...
                     if (connection->was_accepted) {
                        ...
                        CL_LOG(CL_LOG_INFO,
                               "already sent a CCM to client, resetting sub state to CL_COM_RECEIVED_CCM, client wants also to disconnect ...");
                        connection->connection_sub_state = CL_COM_RECEIVED_CCM;
                     }
                  } else {
                     CL_LOG_STR(CL_LOG_ERROR,
                                "Protocol Error: Received connection close message (CCM), but connection is not in expected sub state:",
                                cl_com_get_connection_sub_state(connection));
                  }
               } else {
                  CL_LOG_STR(CL_LOG_ERROR,
                             "Protocol Error: Received connection close message (CCM), but connection is not in expected state:",
                             cl_com_get_connection_state(connection));
               }
               cl_com_free_message(&message);
            ...
            case CL_MIH_DF_CCRM: {
               ...
               if (connection->connection_state == CL_CONNECTED) {
                  if (connection->connection_sub_state == CL_COM_WAIT_FOR_CCRM) {
                     ...
                     CL_LOG_STR(CL_LOG_ERROR,
                                "Protocol Error: Received unexpected connection close response message (CCRM). Connection sub state:",
                                cl_com_get_connection_sub_state(connection));
                  }
               } else {
                  CL_LOG_STR(CL_LOG_ERROR,
                             "Protocol Error: Received unexpected connection close response message (CCRM) in connection state:",
                             cl_com_get_connection_state(connection));
               }
         ...
         if (message->message_state == CL_MS_PROTOCOL) {
            ...
            if (timeout_time <= now.tv_sec) {
               ...
               if (connection->connection_state == CL_CONNECTED &&
                  ...
                  CL_LOG(CL_LOG_INFO,
                         "ignore ack timeout flag is set, but this connection is connected and waiting for ack - continue waiting");
               } else {
                  CL_LOG(CL_LOG_INFO,
                         "ignore ack timeout flag is set and connection is not connected - ignore timeout");
                  if (message->message_id == connection->check_endpoint_mid && connection->check_endpoint_mid != 0) {
                     connection->check_endpoint_mid = 0;
                     connection->check_endpoint_flag = false; /* now the next check can be done */
                  }
               ...
               while (elem) {
                  if (elem->connection->data_flow_type == CL_CM_CT_MESSAGE &&
                     ...
                     if (oldest_connection == nullptr) {
                        ...
                        if (elem->connection->last_transfer_time.tv_sec <
                            oldest_connection->last_transfer_time.tv_sec) {
                           oldest_connection = elem->connection;
                        } else {
                           if (elem->connection->last_transfer_time.tv_sec ==
                               oldest_connection->last_transfer_time.tv_sec) {
                              if (elem->connection->last_transfer_time.tv_usec <
                                  oldest_connection->last_transfer_time.tv_usec) {
                                 oldest_connection = elem->connection;
                              }
                           }
                        }
         ...
         if (handle->max_connection_count_found_connection_to_close) {
            ...
            while (elem) {
               if (elem->connection->data_flow_type == CL_CM_CT_MESSAGE &&
                   elem->connection->connection_state == CL_CONNECTED &&
                   elem->connection->connection_sub_state != CL_COM_WORK) {
                  CL_LOG_STR(CL_LOG_WARNING, "processing close of connection to host:",
                             elem->connection->remote->comp_host);
...
static int cl_commlib_handle_debug_clients(cl_com_handle_t *handle, bool lock_list) {
   ...
   cl_raw_list_lock(handle->debug_client_setup->dc_debug_list);
   CL_LOG_INT(CL_LOG_INFO, "elements to flush:",
              (int) cl_raw_list_get_elem_count(handle->debug_client_setup->dc_debug_list));
   cl_raw_list_unlock(handle->debug_client_setup->dc_debug_list);
   ...
   if (had_data_to_flush && !flushed_client) {
...
static int cl_commlib_external_fd_unregister(cl_com_handle_t *handle, int fd, int lock) {
   ...
   if (handle == nullptr || !cl_com_is_valid_fd(fd) || handle->file_descriptor_list == nullptr) {
      return CL_RETVAL_PARAMS;
   }
...
int cl_com_external_fd_register(cl_com_handle_t *handle,
   ...
   if (handle == nullptr || !cl_com_is_valid_fd(fd) || callback == nullptr) {
      return CL_RETVAL_PARAMS;
   }
...
static int cl_commlib_calculate_statistic(cl_com_handle_t *handle, bool force_update, bool lock_list) {
   ...
   while (elem) {
      ...
      if (con_stat) {
         ...
         handle->statistic->real_bytes_received =
                 handle->statistic->real_bytes_received + con_stat->real_bytes_received;
         con_stat->bytes_sent = 0;
         con_stat->bytes_received = 0;
         con_stat->real_bytes_sent = 0;
         con_stat->real_bytes_received = 0;

         handle->statistic->unsend_message_count = handle->statistic->unsend_message_count +
                                                   cl_raw_list_get_elem_count(elem->connection->send_message_list);
         handle->statistic->unread_message_count = handle->statistic->unread_message_count +
                                                   cl_raw_list_get_elem_count(elem->connection->received_message_list);
      }
      elem = cl_connection_list_get_next_elem(elem);
   }
      ...
      if (message->message_state == CL_MS_READY) {
         ...
         switch (message->message_df) {
            case CL_MIH_DF_CCM:
               ...
               if (connection->connection_state == CL_CONNECTED) {
                  if (connection->connection_sub_state == CL_COM_SENDING_CCM) {
                     ...
                     CL_LOG(CL_LOG_INFO,
                            "now sent a CCM to client, but substate is CL_COM_RECEIVED_CCM, client wants also to disconnect ...");
                  } else {
                     CL_LOG_STR(CL_LOG_ERROR, "Protocol Error: Unexpected substate for CL_CONNECTED connection:",
                                cl_com_get_connection_sub_state(connection));
                  }
               } else {
                  CL_LOG_STR(CL_LOG_ERROR,
                             "Protocol Error: Sent CCM message not in CL_CONNECTED state. Connection state is:",
                             cl_com_get_connection_state(connection));
               }
               break;
            ...
            case CL_MIH_DF_CCRM:
               ...
               if (connection->connection_state != CL_CONNECTED) {
                  CL_LOG_STR(CL_LOG_ERROR,
                             "Protocol Error: Sent CCRM message not in CL_CONNECTED state. Connection state is:",
                             cl_com_get_connection_state(connection));
               }
               break;
                  ...
                  if (response_mid != 0) {
                     ...
                     if (message_elem->message->message_response_id != 0) {
                        if (handle->do_shutdown != 2) {
                           CL_LOG_INT(CL_LOG_INFO, "message response id is set for this message:",
                                      (int) message_elem->message->message_response_id);
                           message_match = 0;
                        } else {
                           ...
                           CL_LOG_INT(CL_LOG_WARNING, "returning response message without request:",
                                      (int) message_elem->message->message_response_id);
                        }
                     }
                  }
...
static int cl_commlib_send_sirm_message(cl_com_connection_t *connection,
   ...
   ret_val = cl_com_setup_message(&sirm_message, connection, sirm_message_data, sirm_message_size, CL_MIH_MAT_NAK, 0,
                                  0);
   if (ret_val != CL_RETVAL_OK) {
      return ret_val;
   }
...
static int cl_commlib_send_ccrm_message(cl_com_connection_t *connection) {
   ...
   ret_val = cl_com_setup_message(&ccrm_message, connection, ccrm_message_data, ccrm_message_size, CL_MIH_MAT_NAK, 0,
                                  0);
   if (ret_val != CL_RETVAL_OK) {
      return ret_val;
   }
...
int cl_commlib_send_message(cl_com_handle_t *handle,
    ...
    *
    */
   if (mid == nullptr && !wait_for_ack && cl_com_create_threads != CL_NO_THREAD) {
      ...
      CL_LOG_STR_STR_INT(CL_LOG_INFO, "add new message for:      ", unique_hostname, component_name,
                         (int) component_id);
...
static void *cl_com_handle_service_thread(void *t_conf) {
   ...
   while (do_exit == 0) {
      ...
      CL_LOG(CL_LOG_INFO, "wait for event ...");
      if ((ret_val = cl_thread_wait_for_event(thread_config, handle->select_sec_timeout,
                                              handle->select_usec_timeout)) != CL_RETVAL_OK) {
         switch (ret_val) {
            case CL_RETVAL_CONDITION_WAIT_TIMEOUT:
               CL_LOG(CL_LOG_INFO, "condition wait timeout");
...
static void *cl_com_handle_read_thread(void *t_conf) {
   ...
   while (do_exit == 0) {
      ...
      ret_val = cl_com_open_connection_request_handler(poll_handle, handle, handle->select_sec_timeout,
                                                       handle->select_usec_timeout, CL_R_SELECT);
      switch (ret_val) {
      ...
      while (elem) {
         switch (elem->connection->connection_state) {
            ...
            case CL_CONNECTING: {
               if (elem->connection->data_read_flag == CL_COM_DATA_READY) {
                  return_value = cl_com_connection_complete_request(handle->connection_list, elem,
                                                                    handle->open_connection_timeout, CL_R_SELECT);
...
static int cl_com_handle_ccm_process(cl_com_connection_t *connection) {
   ...
   if (connection->connection_state == CL_CONNECTED && connection->connection_sub_state == CL_COM_RECEIVED_CCM) {
      if (cl_raw_list_get_elem_count(connection->send_message_list) == 0 &&
         ...
         CL_LOG_INT(CL_LOG_INFO, "receive buffer:",
                    (int) cl_raw_list_get_elem_count(connection->received_message_list));
...
static void *cl_com_handle_write_thread(void *t_conf) {
   ...
   while (do_exit == 0) {
      ...
      /* do write select */
      ret_val = cl_com_open_connection_request_handler(poll_handle, handle, handle->select_sec_timeout,
                                                       handle->select_usec_timeout, CL_W_SELECT);
      switch (ret_val) {
         case CL_RETVAL_SELECT_TIMEOUT:
            ...
            while (elem) {
               switch (elem->connection->connection_state) {
                  ...
                  case CL_ACCEPTING: {
                     ...
                     if (elem->connection->fd_ready_for_write == CL_COM_DATA_READY &&
                         elem->connection->data_write_flag == CL_COM_DATA_READY) {
                        return_value = cl_com_connection_complete_accept(elem->connection,
                                                                         handle->open_connection_timeout);
                        if (return_value != CL_RETVAL_OK) {
                           if (return_value != CL_RETVAL_UNCOMPLETE_READ &&
                               return_value != CL_RETVAL_UNCOMPLETE_WRITE &&
                  ...
                  case CL_CONNECTING: {
                     if (elem->connection->fd_ready_for_write == CL_COM_DATA_READY &&
                         elem->connection->data_write_flag == CL_COM_DATA_READY) {
                        return_value = cl_com_connection_complete_request(handle->connection_list, elem,
                                                                          handle->open_connection_timeout, CL_W_SELECT);
                        if (return_value != CL_RETVAL_OK) {
                           if (return_value != CL_RETVAL_UNCOMPLETE_READ &&
                               return_value != CL_RETVAL_UNCOMPLETE_WRITE &&
                               return_value != CL_RETVAL_SELECT_ERROR) {
