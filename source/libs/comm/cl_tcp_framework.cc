   ...
   if ((private_com = cl_com_tcp_get_private(connection)) != nullptr) {
      if (private_com->sockfd < 0) {
         ...
         *fd = private_com->pre_sockfd;
         ...
         CL_LOG_INT(CL_LOG_INFO, "return final sockfd: ", private_com->sockfd);
         *fd = private_com->sockfd;
      }
      return CL_RETVAL_OK;
   }
...
int cl_com_tcp_open_connection(cl_com_connection_t *connection, int timeout) {
   ...
   if (connection->connection_sub_state == CL_COM_OPEN_INIT) {
      ...
      switch (connection->tcp_connect_mode) {
         case CL_TCP_DEFAULT: {
            /* create socket */
            if ((private_com->sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
         ...
         case CL_TCP_RESERVED_PORT: {
            ...
            if ((private_com->sockfd = rresvport(&res_port)) < 0) {
               CL_LOG(CL_LOG_ERROR, "could not create reserved port socket");
               private_com->sockfd = -1;
               cl_commlib_push_application_error(CL_LOG_ERROR, CL_RETVAL_CREATE_SOCKET,
                                                 MSG_CL_TCP_FW_RESERVED_SOCKET_ERROR);
      ...
      if (private_com->sockfd < 3) {
         CL_LOG_INT(CL_LOG_WARNING, "The file descriptor is < 3. Will dup fd to be >= 3! fd value: ", private_com->sockfd);
         ret = sge_dup_fd_above_stderr(&(private_com->sockfd));
         if (ret != 0) {
            ...
            cl_commlib_push_application_error(CL_LOG_ERROR, CL_RETVAL_DUP_SOCKET_FD_ERROR,
                                              MSG_CL_COMMLIB_CANNOT_DUP_SOCKET_FD);
      ...
      if (fcntl(private_com->sockfd, F_SETFL, O_NONBLOCK) != 0) {
      ...
      /* set address  */
      memset((char *) &(private_com->client_addr), 0, sizeof(struct sockaddr_in));
   ...
   if (connection->connection_sub_state == CL_COM_OPEN_CONNECT) {
      ...
      errno = 0;
      i = connect(private_com->sockfd, (struct sockaddr *) &(private_com->client_addr), sizeof(struct sockaddr_in));
      my_error = errno;
      if (i == 0 || my_error == EISCONN) {
         ...
         switch (my_error) {
            case ECONNREFUSED: {
               ...
               CL_LOG_INT(CL_LOG_ERROR, "connection refused to port ", private_com->connect_port);
               shutdown(private_com->sockfd, 2);
               close(private_com->sockfd);
               private_com->sockfd = -1;
            ...
            case EADDRNOTAVAIL: {
               /* can't open connection */
               CL_LOG_INT(CL_LOG_ERROR, "address not available for port ", private_com->connect_port);
               shutdown(private_com->sockfd, 2);
               close(private_com->sockfd);
               private_com->sockfd = -1;
               cl_commlib_push_application_error(CL_LOG_ERROR, CL_RETVAL_CONNECT_ERROR, strerror(my_error));
   ...
   if (connection->connection_sub_state == CL_COM_OPEN_CONNECT_IN_PROGRESS) {
      ...
      getsockopt(private_com->sockfd, SOL_SOCKET, SO_ERROR, (void*)&socket_error, &socklen);
#else
      getsockopt(private_com->sockfd, SOL_SOCKET, SO_ERROR, &socket_error, &socklen);
#endif
...
#else
      if (setsockopt(private_com->sockfd, IPPROTO_TCP, TCP_NODELAY, &on, sizeof(int)) != 0)
#endif
      {
...
int cl_com_tcp_read_GMSH(cl_com_connection_t *connection, unsigned long *only_one_read) {
   ...
   /* parse header */
   retval = cl_xml_parse_GMSH(connection->data_read_buffer, connection->data_read_buffer_pos,
                              connection->read_gmsh_header, &processed_data);
...
static int cl_com_tcp_connection_request_handler_setup_finalize(cl_com_connection_t *connection) {
   ...
   CL_LOG_INT(CL_LOG_INFO, "server fd:", private_com->sockfd);
...
int cl_com_tcp_connection_request_handler_setup(cl_com_connection_t *connection, bool only_prepare_service) {
   ...
   serv_addr.sin_port = htons(private_com->server_port);
   ...
   if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) {
      shutdown(sockfd, 2);
      close(sockfd);
      CL_LOG_INT(CL_LOG_ERROR, "could not bind server socket port:", private_com->server_port);
   ...
   if (private_com->server_port == 0) {
      ...
      if (getsockname(sockfd, (struct sockaddr *) &serv_addr, &length) == -1) {
         shutdown(sockfd, 2);
         close(sockfd);
         CL_LOG_INT(CL_LOG_ERROR, "could not bind random server socket port:", private_com->server_port);
         return CL_RETVAL_BIND_SOCKET;
      }
      private_com->server_port = ntohs(serv_addr.sin_port);
      CL_LOG_INT(CL_LOG_INFO, "random server port is:", private_com->server_port);
   }
                                               ...
                                               cl_select_method_t select_mode)
{
   ...
   if (service_connection != nullptr && do_read_select != 0) {
      ...
      private_com = cl_com_tcp_get_private(service_connection);
      ...
      if (private_com->sockfd == -1 && private_com->pre_sockfd != -1) {
               ...
               if (connection != nullptr) {
                  ...
                  if ((ufds[fd_index].revents & (POLLERR | POLLHUP | POLLNVAL)) && connection != service_connection) {
                     if (ufds[fd_index].revents & POLLNVAL) {
                        CL_LOG_INT(CL_LOG_WARNING, "poll() revents POLLNVAL is set - checking file descriptor: ",
                                   (int) ufds[fd_index].fd);
                     }
                     if (ufds[fd_index].revents & POLLERR) {
                        CL_LOG_INT(CL_LOG_WARNING, "poll() revents POLLERR is set - checking file descriptor: ",
                                   (int) ufds[fd_index].fd);
                     }
                     if (ufds[fd_index].revents & POLLHUP) {
                        CL_LOG_INT(CL_LOG_WARNING, "poll() revents POLLHUP is set - checking file descriptor: ",
                                   (int) ufds[fd_index].fd);
                     }
                     ...
                     get_sock_opt_error = getsockopt(con_private->sockfd,SOL_SOCKET, SO_ERROR, (void*)&socket_error, &socklen);
#else
                     get_sock_opt_error = getsockopt(con_private->sockfd, SOL_SOCKET, SO_ERROR, &socket_error,
                                                     &socklen);
#endif
                     if (socket_error != 0 || get_sock_opt_error != 0) {
                        connection->connection_state = CL_CLOSING;
