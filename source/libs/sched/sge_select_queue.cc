...
static bool
add_pe_slots_to_category(category_use_t *use_category, uint32_t *max_slotsp, lListElem *pe,
...
static void
parallel_add_queue_to_gdil(sge_assignment_t *a, const char *qname, const char *eh_name, const lListElem *qep, uint32_t slots, bool is_master_queue) {
   if (a->gdil == nullptr) {
...
parallel_tag_queues_suitable4job(sge_assignment_t *a, category_use_t *use_category, int *available_slots)
{
   ...
   if (gslots < a->slots) {
         ...
         for_each_rw (hep, a->host_list) {
            ...
            if (lGetElemHost(a->queue_list, QU_qhostname, eh_name)) {
               ...
               if (hslots >= minslots) {
                  ...
                  int rqs_hslots = 0, maxslots, slots;
      ...
      } while (allocation_rule == ALLOC_RULE_ROUNDROBIN && !done && last_accu_host_slots != accu_host_slots);
