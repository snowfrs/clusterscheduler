...
parallel_tag_hosts_queues(sge_assignment_t *a, lListElem *hep, int *slots, bool need_master,
                          ...
                          lList **unclear_cqueue_list, int min_host_slots, int max_host_slots);
...
static dispatch_t
sequential_global_time(uint64_t *start_time, sge_assignment_t *a, int *violations);
...
static dispatch_t
rc_time_by_slots(sge_assignment_t *a, lList *requested, const lList *load_attr, const lList *config_attr, const lList *actual_attr,
                 const lListElem *host, lListElem *queue, bool allow_non_requestable, dstring *reason, int slots,
                 uint32_t layer, double lc_factor, uint32_t tag, uint64_t *start_time, const char *object_name);
...
ri_slots_by_time(const sge_assignment_t *a, int *slots, const lList *rue_list, lListElem *request,
                 ...
                 uint32_t layer, double lc_factor, dstring *reason, bool allow_non_requestable, bool no_centry,
...
static int
resource_cmp(uint32_t relop, double req, double src_dl);

static bool
job_is_forced_centry_missing(const sge_assignment_t *a, const lListElem *queue_or_host, bool is_qinstance);

static void
clear_resource_tags(lList *resources, uint32_t max_tag);
static void
clear_resource_tags(lListElem *job, uint32_t max_tag);
...
print_tagged4schedule(const lListElem *qinstance) {
   ...
   if (DPRINTF_IS_ACTIVE) {
      if (qinstance != nullptr) {
         uint32_t bitmask = lGetUlong(qinstance, QU_tagged4schedule);
...
void assignment_init(sge_assignment_t *a, lListElem *job, lListElem *ja_task, lList *load_adjustments)
{
   ...
   a->do_binding_on_any_host = mconf_schedule_on_any_host();
   a->filter_first_core = strcasecmp(mconf_get_binding_filter().c_str(), FIRST_CORE) == 0;
}

/**
 * @brief initializes the AR related fields of the assignment
 *
 * Initializes the ar_list and the ar attributes.
 * Must be called *after* assignment_init().
 *
 * @param a  the assignment
 * @param ar_list  the list of advance reservations
 */
void assignment_init_ar(sge_assignment_t *a, lList *ar_list) {
   // ar_list can be called with nullprt from prepare_resource_schedules()
   // do not handle not finding the AR as an error then
   a->ar_id = lGetUlong(a->job, JB_ar);
   if (ar_list != nullptr) {
      a->ar_list = ar_list;
      if (a->ar_id > 0) {
...
assignment_init_pe_parse_check_allocation_rules(sge_assignment_t *a, int check_slots = 0) {
   ...
   // When we pass check_slots != 0, we want to check these slots.
   // Otherwise, we work on the assignment structure.
   int slots = check_slots > 0 ? check_slots : a->slots;

   // Parse the allocation rule(s) once, use the maximum number of slots we might want to assign (a->slots)
   DPRINTF("parsing allocation rules, slots: %d\n", slots);
   int mallocation_parsed = 0, allocation_parsed = 0;
   int master_slots = 0;
   if (a->mallocation_rule != nullptr) {
      mallocation_parsed = pe_allocation_rule_slots(a->mallocation_rule, slots, false);
      if (mallocation_parsed == 0) {
         // error
         DRETURN(false);
      } else {
         DPRINTF("   -> master allocation_rule: %s, parsed: %d\n", a->mallocation_rule, mallocation_parsed);
         if (ALLOC_RULE_IS_FIXED(mallocation_parsed)) {
            master_slots = mallocation_parsed;
         } else {
            master_slots = 1;
         }
      }
   }
   int slave_slots = slots - master_slots;
   allocation_parsed = pe_allocation_rule_slots(a->allocation_rule, slave_slots);
   if (allocation_parsed == 0) {
      // error
      DRETURN(false);
   } else {
      DPRINTF("   -> allocation_rule: %s, parsed: %d\n", a->allocation_rule, allocation_parsed);
   }

   // This is not only a check - we want to store the parsed results in the assignment struct.
   if (check_slots == 0) {
      a->mallocation_parsed = mallocation_parsed;
      a->allocation_parsed = allocation_parsed;
   }
...
assignment_init_pe(sge_assignment_t *a, lListElem *pe) {
   ...
   if (a->job != nullptr) {
      // We can specify the -par switch either globally.
      ...
      if (jrs_allocation_rule != nullptr) {
         ...
         // Or we can specify the -par switch for master and/or slave scope.
         jrs_allocation_rule = job_get_allocation_rule(a->job, JRS_SCOPE_MASTER);
         if (jrs_allocation_rule != nullptr) {
            a->mallocation_rule = jrs_allocation_rule;
         }
         jrs_allocation_rule = job_get_allocation_rule(a->job, JRS_SCOPE_SLAVE);
         if (jrs_allocation_rule != nullptr) {
            // If the -par switch is *only* used for the slave scope,
            // then for the master scope the PE setting is applied.
            if (a->mallocation_rule == nullptr) {
               a->mallocation_rule = a->allocation_rule;
            }
            a->allocation_rule = jrs_allocation_rule;
         }
      }
      ...
      // Parse the allocation rule(s) once, use the maximum number of slots we might want to assign (a->slots)
      if (a->slots > 0) {
         assignment_init_pe_parse_check_allocation_rules(a);
      }
   }
...
static bool
is_not_better(sge_assignment_t *a, uint32_t viola_best, uint64_t tt_best, uint32_t viola_this, uint64_t tt_this)
{
...
sge_select_parallel_environment(sge_assignment_t *best, const lList *pe_list)
{
   ...
   if (best->ar_id == 0) {
      ...
      // following code
      pe_request = lGetString(best->ar, AR_granted_pe);
   }
...
parallel_reservation_max_time_slots(sge_assignment_t *best, int *available_slots)
{
   ...
   if (best->slots == 0) {
      ...
      assignment_init_pe_parse_check_allocation_rules(&tmp_assignment);
   }
...
parallel_maximize_slots_pe(sge_assignment_t *best, int *available_slots)
{
   ...
   // @todo add a shortcut: if first == last, do not do all the stuff with binary search, etc.,
   //       just set the slots and call parallel_assignment()!
   ...
   if (alg == SCHEDD_PE_BINARY) {
      ...
      do {
         ...
         assignment_init_pe_parse_check_allocation_rules(&tmp);
...
/* global */
   ...
   ret = rc_time_by_slots(&a, requested_attr, load_attr, config_attr, actual_attr,
                     nullptr, nullptr, allow_non_requestable, nullptr, slots,
                          DOMINANT_LAYER_HOST, lc_factor, GLOBAL_TAG,
...
/* host */
   if(ret == DISPATCH_OK || ret == DISPATCH_MISSING_ATTR){
      ...
      ret = rc_time_by_slots(&a, requested_attr, load_attr, config_attr,
                             actual_attr, host, nullptr, allow_non_requestable, nullptr,
...
/* queue */
     if((ret == DISPATCH_OK || ret == DISPATCH_MISSING_ATTR) && queue){
         ...
         ret = rc_time_by_slots(&a, requested_attr, nullptr, config_attr, actual_attr, nullptr,
                                queue, allow_non_requestable, nullptr, slots, DOMINANT_LAYER_QUEUE,
                                0, QUEUE_TAG, &start_time,  lGetString(queue, QU_full_name));
      }
   }
...
rc_time_by_slots(sge_assignment_t *a, lList *requested, const lList *load_attr, const lList *config_attr,
                 ...
                 dstring *reason, int slots, uint32_t layer, double lc_factor, uint32_t tag,
                 uint64_t *start_time, const char *object_name)
{
   DENTER(TOP_LAYER);
   ...
   if (slots != -1) {
      ...
      if (ret == DISPATCH_OK && *start_time == DISPATCH_TIME_QUEUE_END) {
         ...
         uint64_t max_time = std::max(latest_time, tmp_start);
...
dispatch_t sge_queue_match_static(const sge_assignment_t *a, lListElem *queue, bool need_master)
{
   ...
   if (a->ar_id != 0) {
      ...
      if (lGetSubStr(a->ar, QU_full_name, qinstance_name, AR_reserved_queues) == nullptr) {
         uint32_t ar_id = lGetUlong(a->ar, AR_id);
   ...
   if (global_hard_queue_list != nullptr) {
   ...
   } else if (a->pe != nullptr) {
      // we do this only for parallel jobs
      ...
      // no need to check this if we already have our master task
      if (need_master && master_hard_queue_list != nullptr) {
      ...
      if (slave_hard_queue_list != nullptr) {
         if (qref_list_cq_rejected(slave_hard_queue_list, lGetString(queue, QU_qname),
            ...
            // lClearUlongBitMask(queue, QU_tagged4schedule, TAG4SCHED_SLAVE);
         }
      }
...
compute_soft_violations(const sge_assignment_t *a, const lListElem *host, lListElem *queue, int violation,
                        ...
                        const lList *actual_attr, uint32_t layer, double lc_factor, uint32_t tag)
{
...
sge_host_match_static(const sge_assignment_t *a, const lListElem *host)
{
   ...
   if (a->ar_id != 0) {
      if (a->ar != nullptr) {
         const lListElem *res_host = lGetSubHost(a->ar, EH_name, eh_name, AR_reserved_hosts);
         if (res_host == nullptr) {
            DPRINTF("Host \"%s\" was not reserved by advance reservation %d\n", eh_name, a->ar_id);
            schedd_mes_add(a->monitor_alpp, a->monitor_next_run, a->job_id,
                           SCHEDD_INFO_HNOTARRESERVED_SI, eh_name, a->ar_id);
            DRETURN(DISPATCH_NEVER_CAT);
         }
      } else {
         /* should never happen */
   ...
   *reason = 0;
   ...
   if (threshold != nullptr) {
      ...
      for_each_ep(tep, threshold) {
         ...
         uint32_t dom_val;             /* dominance as u_long */
...
dispatch_t cqueue_match_static(const char *cqname, sge_assignment_t *a) {
   ...
   if (a->ar_id != 0) {
      if (a->ar != nullptr) {
         const lListElem *gdil_ep = lGetSubStr(a->ar, QU_qname, cqname, AR_reserved_queues);
         if (gdil_ep == nullptr) {
            DPRINTF("Cluster Queue \"%s\" was not reserved by advance reservation %d\n", cqname, a->ar_id);
            schedd_mes_add(a->monitor_alpp, a->monitor_next_run, a->job_id,
                           SCHEDD_INFO_QNOTARRESERVED_SI, cqname, a->ar_id);
            DRETURN(DISPATCH_NEVER_CAT);
         }
      } else {
         /* should never happen */
...
sequential_tag_queues_suitable4job(sge_assignment_t *a)
{
   ...
   uint64_t tt_best = std::numeric_limits<uint64_t>::max();
   uint32_t violations_best = std::numeric_limits<uint32_t>::max();

   dispatch_t result;
   uint64_t tt_global = a->start;
   ...
   for_each_rw(qep, a->queue_list) {
      ...
      if (a->ar == nullptr) {
         ...
         if ((result = rqs_by_slots(a, cqname, eh_name, &tt_rqs, &is_global,
                  &rue_string, &limit_name, &rule_name, got_solution?tt_best:std::numeric_limits<uint64_t>::max())) != DISPATCH_OK) {
      ...
      if (result == DISPATCH_OK) {
         uint64_t this_tt = 0, this_violations = 0;
         ...
         DPRINTF("    set Q: %s number=1 when=" sge_u64" violations=" sge_u64 "\n", qname, this_tt, this_violations);
         ...
         if (!a->is_reservation) {
            ...
            /* earlier start time has higher preference than lower soft violations */
            if (a->ar == nullptr && (tt_global >= MAX(MAX(tt_queue, tt_host), tt_rqs)) && !a->is_soft) {
               /* we found our queue */
               if (best_qep != nullptr) {
                  lSetUlong(best_qep, QU_tag, 0);
               }
...
add_pe_slots_to_category(sge_assignment_t *a, category_use_t *use_category, uint32_t *max_slotsp,
   ...
   DPRINTF("add_pe_slots_to_category\n");
   if (use_category->cache != nullptr) {
      use_category->possible_pe_slots = (uint32_t *)lGetRef(use_category->cache, CCT_pe_job_slots);
   }
   ...
   if (use_category->possible_pe_slots == nullptr) {
      ...
      *max_slotsp = 0;
      for (slots = min_slots; slots <= max_slots; slots++) {
         // Sort out slot numbers that would conflict with the allocation rule(s).
         if (!assignment_init_pe_parse_check_allocation_rules(a, slots)) {
            continue;
         }
         ...
         if (use_category->possible_pe_slots == nullptr) {
            use_category->possible_pe_slots = (uint32_t *)sge_malloc((max_slots - min_slots + 1) * sizeof(uint32_t));
            if (use_category->possible_pe_slots == nullptr) {
               DRETURN(false);
            }
         }
...
static int get_soft_violations(sge_assignment_t *a, lListElem *hep, lListElem *qep)
{
   ...
   violations = compute_soft_violations(a, nullptr, nullptr, 0,
   ...
   violations = compute_soft_violations(a, hep, nullptr, violations,
   ...
   violations = compute_soft_violations(a, nullptr, qep, violations,
           nullptr,
...
static void
parallel_add_queue_to_gdil(sge_assignment_t *a, const char *qname, const char *eh_name, const lListElem *qep,
                           uint32_t slots, bool is_master_queue) {
   DENTER(TOP_LAYER);

   DPRINTF("add %s queue %s to GDIL, " sge_u32 " slots\n", is_master_queue ? "master" : "slave", qname, slots);
...
parallel_revert_rqs(sge_assignment_t *a, const char *eh_name, std::vector<queue_slots_t> &queue_slots,
   ...
   for (queue_slots_t &queue_slot : queue_slots) {
      ...
      int slots = queue_slot.slots;
      if (slots != 0) {
         if (a->ar_id == 0 && !a->is_advance_reservation) {
 ...
 * @param accu_slots           - the number of slots we have assigned so far
 * @param minslots_master      - the minimum number of slots we need to find in queue tagging and will assign on a host
 * @param maxslots_master      - the maximum number of slots we will assign as maximum in one round
 * @param minslots_slaves      - the minimum number of slots we need to find in queue tagging and will assign on a host
 * @param maxslots_slaves      - the maximum number of slots we will assign as maximum in one round
 * @param minslots_for_tagging - the minimum number of slots we want to find on a host in queue tagging
 * @param maxslots_for_tagging - the maximum number of slots we want to find on a host in queue tagging
 * @return
 */
static bool
parallel_get_minmax_slots_for_allocation_rules(sge_assignment_t *a, bool have_master_host, int accu_slots,
                                               int &minslots_master, int &maxslots_master,
                                               int &minslots_slaves, int &maxslots_slaves,
                                               int &minslots_for_tagging, int &maxslots_for_tagging) {
   DENTER(TOP_LAYER);

   // We return if a round robin run is still required:
   //   - When we do not yet have a master task and the master allocation rule is $round_robin.
   //   - If the global or the slave allocation rule is $round_robin.
   bool do_round_robin = false;

   int remaining_needed = a->slots - accu_slots;
   minslots_master = 0;
   maxslots_master = remaining_needed;
   minslots_slaves = 0;
   maxslots_slaves = remaining_needed;
   minslots_for_tagging = 1;
   maxslots_for_tagging = remaining_needed;

   // If we do not yet have a master task, shall we try to use the host as slave only host?
   bool do_slave_only = true;
   bool have_master_allocation_rule = false;

   // If we shall find a master host.
   if (!have_master_host && a->mallocation_rule != nullptr) {
      have_master_allocation_rule = true;
      // When we have a fixed allocation rule (or $pe_slots), we need exactly that number of slots.
      if (ALLOC_RULE_IS_FIXED(a->mallocation_parsed)) {
         minslots_master = a->mallocation_parsed;
         maxslots_master = a->mallocation_parsed;
         minslots_for_tagging = a->mallocation_parsed;
         maxslots_for_tagging = a->mallocation_parsed;
         // If what the master task will give us is the number of slots we still need to find,
         // do not allocate any more slots on slave only hosts.
         if (remaining_needed == a->mallocation_parsed) { // @todo <= would be an error!
            do_slave_only = false;
         }
      } else {
         minslots_master = 1;
         if (a->mallocation_parsed == ALLOC_RULE_ROUNDROBIN) {
            do_round_robin = true;
            // $round_robin: We will allocate one slot per round, but tagging will try to put onto the host
            //               as much as possible.
            maxslots_master = 1;
            // If we just need a single slot and this is the master task
            // do not allocate any more slots on slave only hosts.
   ...
   if (do_slave_only) {
      // When we have a fixed allocation rule (or $pe_slots), we need exactly that number of slots.
      if (ALLOC_RULE_IS_FIXED(a->allocation_parsed)) {
         if (have_master_allocation_rule) {
            minslots_for_tagging = MIN(minslots_for_tagging, a->allocation_parsed);
            maxslots_for_tagging= MAX(maxslots_for_tagging, a->allocation_parsed);
         } else {
            minslots_for_tagging = a->allocation_parsed;
            maxslots_for_tagging = a->allocation_parsed;
         }
         minslots_slaves = a->allocation_parsed;
         maxslots_slaves = a->allocation_parsed;
      } else {
         minslots_slaves = 1;
         if (a->allocation_parsed == ALLOC_RULE_ROUNDROBIN) {
            do_round_robin = true;
            maxslots_slaves = 1;
         }
      }
   ...
   DPRINTF("   minslots_master:  %d, maxslots_master:  %d\n", minslots_master, maxslots_master);
   DPRINTF("   minslots_slaves:  %d, maxslots_slaves:  %d\n", minslots_slaves, maxslots_slaves);
   DPRINTF("   minslots_tagging: %d, maxslots_tagging: %d\n", minslots_for_tagging, maxslots_for_tagging);
   DPRINTF("   do_round_robin:   %s\n", do_round_robin ? "true" : "false");

   DRETURN(do_round_robin);
}
...
parallel_allocate_queue_slots(sge_assignment_t *a, const char *eh_name, bool have_master_host, bool &got_master_queue,
   ...
   for (qep = lGetElemHostFirstRW(a->queue_list, QU_qhostname, eh_name, &iter); qep;
      ...
      int slots = MIN(lGetUlong(qep, QU_tag), maxslots - rqs_hslots);
      ...
      if (!have_master_host && !got_master_queue) {
         if (!lMatchUlongBitMask(qep, QU_tagged4schedule, TAG4SCHED_MASTER)) {
             ...
             */
            DPRINTF("   -> not suited as master queue, accu_host_slots = %d, rqs_hslots = %d, slots = %d, a->slots = %d\n",
                    accu_host_slots, rqs_hslots, slots, a->slots);
            if (accu_host_slots + rqs_hslots + slots == a->slots) {
               // If allocating the slots (as slave tasks, as the qinstance is not suitable as master queue)
               // would provide enough slots to satisfy the pe request,
               // we need to reduce the slot count by one to leave space for the master task (which must come from
               // a different qinstance).
               DPRINTF("   -> reducing slots to %d\n", slots - 1);
               slots--;
            }
         } else {
      ...
      if (slots > 0) {
         // Remember the queue instance and its allocation for creating the GDIL later on.
         queue_slots_t qs{};
         qs.qname = qname;
         qs.slots = slots;
         if (a->is_soft) {
         ...
         if (!got_master_queue && lMatchUlongBitMask(qep, QU_tagged4schedule, TAG4SCHED_MASTER)) {
            // We got our master queue, insert it as first element.
            got_master_queue = true;
            qs.is_master_queue = true;
            queue_slots.insert(queue_slots.begin(), qs);
         } else {
...
parallel_tag_queues_suitable4job(sge_assignment_t *a, category_use_t *use_category, int *available_slots)
{
   ...
   parallel_global_slots(a, &gslots);

   if (gslots < a->slots) {
      best_result = DISPATCH_NEVER_CAT;
      DPRINTF("GLOBAL will <category_never> get us %d slots\n", gslots);
   } else {
      ...
      // This avoids tons of malloc/free calls.
      dstring rule_name = DSTRING_INIT;
      ...
      do {
         ...
         for_each_rw (hep, a->host_list) {
             ...
             */
            // @todo (CS-456) cheaper check: lGetUlong(EH_seq_no) == std::numeric_limits<uint32_t>::max(), we could possibly break out of the host loop
            ...
            //       the number of available slots should already be in QU_tag?
            //       hslots = sum(QU_tag)?
            ...
            int minslots_master, maxslots_master;
            int minslots_slaves, maxslots_slaves;
            int minslots_for_tagging, maxslots_for_tagging;
            do_round_robin = parallel_get_minmax_slots_for_allocation_rules(a, have_master_host, accu_host_slots,
                             minslots_master, maxslots_master, minslots_slaves, maxslots_slaves,
                             minslots_for_tagging, maxslots_for_tagging);

            int hslots = 0;
            // @todo only if there is actually a qinstance on the host - but hasn't this not been checked before?
            //       wouldn't eh_name be in some skip list then?
            if (lGetElemHost(a->queue_list, QU_qhostname, eh_name)) {
               ...
               DPRINTF("===> HOST: %s %p <=== need_master: %d is_master_host: %d (%p)\n", eh_name, (void *)hep,
                       need_master, is_master_host, (void *)master_host);
               // @todo Why do we call parallel_tag_hosts_queues,
               //       when we have round_robin and are in the second or higher round?
               //       ==> store the possible amount somewhere (QU_tag?)
               parallel_tag_hosts_queues(a, hep, &hslots, need_master, is_master_host,
                                         &suited_as_master_host, use_category, &unclear_cqueue_list,
                                         minslots_for_tagging, maxslots_for_tagging);
               if (hslots >= minslots_for_tagging) {
                  bool got_master_queue = have_master_host;
                  int slots = 0;
                  std::vector<queue_slots_t> queue_slots;

                  if (need_master && minslots_master > 0) {
                     // We still need to find a master host
                     ...
                     if (slots == 0) {
                        if (minslots_slaves > 0) {
                           ...
                           // This host is not suited as master host.
                           // Try to allocate slots with slave allocation rule.
                           // Pretend that we already have the master task (we will hopefully find it on another host)
                  ...
                  // We could allocate slots on this host.
                  if (hslots > 0) {
                     DPRINTF("     -> got %d slots on host %s\n", hslots, eh_name);
                     ...
                     // Create the GDIL and book soft violations from our temporary queue list.
                     for (queue_slots_t &queue_slot : queue_slots) {
                        lListElem *gdil_ep = lGetElemStrRW(a->gdil, JG_qname, queue_slot.qname);
                        if (gdil_ep == nullptr) {
                     ...
                     // Sum up the total number of slots found so far.
             ...
             * at least when accu_host_slots < a->slots
            ...
            // exit is possible when we got the full slot amount
         ...
         // @todo additional termination condition: if we didn't find the master host in the first round, we will never find it!
      } while (do_round_robin && !done && last_accu_host_slots != accu_host_slots);
...
static void host_clear_qinstance_tags(lList *queue_list, const char *host_name, uint32_t bitmask) {
...
static void
host_or_queue_clear_tags(const char *object_name, lListElem *queue, lList *queue_list, uint32_t bitmask) {
   if (queue != nullptr) {
...
static dispatch_t
parallel_host_slots(sge_assignment_t *a, int *slots, lListElem *hep, bool need_master,
   ...
   if (!(rejected_as_master && rejected_as_slave) &&
      ...
      if (lGetPosViaElem(hep, EH_load_correction_factor, SGE_NO_ABORT) >= 0) {
         uint32_t ulc_factor = lGetUlong(hep, EH_load_correction_factor);
         if (ulc_factor > 0) {
      ...
      result = parallel_rc_slots_by_time(a, &hslots, config_attr, actual_attr, load_list,
                               false, hep, nullptr, DOMINANT_LAYER_HOST, lc_factor, HOST_TAG,
                                         need_master, is_master_host, found_master_host, false, eh_name, false);
   ...
   *slots = hslots;
   ...
   if (result == DISPATCH_OK) {
      DPRINTF("\tparallel_host_slots(%s) returns %d\n", eh_name, hslots);
   } else {
                          ...
                          lList **unclear_cqueue_list, int min_host_slots, int max_host_slots)
{
   ...
   parallel_host_slots(a, &hslots, hep, need_master, is_master_host, found_master_host, false);
   ...
   if (hslots >= min_host_slots) {
      ...
      for (next_queue = lGetElemHostFirstRW(a->queue_list, QU_qhostname, eh_name, &queue_iterator);
         ...
         result = parallel_queue_slots(a, qep, &qslots, need_master, is_master_host, found_master_host, false);

         if (result == DISPATCH_OK && qslots > 0) {
            /* could this host be a master host */
            if (!suited_as_master_host && lMatchUlongBitMask(qep, QU_tagged4schedule, TAG4SCHED_MASTER)) {
               DPRINTF("HOST %s can be master host because of queue %s\n", eh_name, qname);
               suited_as_master_host = true;
            }

            DPRINTF("QUEUE %s TIME: %d + %d -> %d -> %d (%d soft violations)\n", qname,
...
dispatch_t sge_sequential_assignment(sge_assignment_t *a)
{
   ...
   if (result == DISPATCH_OK) {
      const lListElem *qep;
      uint64_t job_start_time = std::numeric_limits<uint64_t>::max();
      uint32_t min_soft_violations = std::numeric_limits<uint32_t>::max();
      const lListElem *best_queue = nullptr;

      if (a->is_reservation) {
         for_each_ep(qep, a->queue_list) {
            if (lGetUlong(qep, QU_tag) != 0) {
               uint64_t temp_job_start_time = lGetUlong64(qep, QU_available_at);
...
parallel_assignment(sge_assignment_t *a, category_use_t *use_category, int *available_slots)
{
   ...
   if (a->ar_id == 0 && (ret = parallel_available_slots(a, &pslots)) != DISPATCH_OK) {
   ...
   if (a->slots > pslots) {
      *available_slots = pslots;
      schedd_mes_add(a->monitor_alpp, a->monitor_next_run, a->job_id,
                     SCHEDD_INFO_PESLOTSNOTINRANGE_SI, a->pe_name, pslots);
      DRETURN(DISPATCH_NEVER_CAT);
   }
                     ...
                     bool is_master_queue, bool &found_master_host, bool allow_non_requestable)
{
   ...
   if (result == DISPATCH_OK) {
      if (a->ar_id == 0 && !a->is_advance_reservation) {
         ...
         result = parallel_rqs_slots_by_time(a, &lslots, qep, need_master, is_master_queue);
      }
   }
   ...
   if (result == DISPATCH_OK) {
      ...
      result = parallel_rc_slots_by_time(a, &qslots,
                                         config_attr, actual_attr, nullptr, true, nullptr, qep,
...
sequential_queue_time(uint64_t *start, sge_assignment_t *a, int *violations, lListElem *qep)
{
   ...
   result = rc_time_by_slots(a, hard_requests, nullptr, config_attr, actual_attr,
                            nullptr, qep, false, &reason, 1, DOMINANT_LAYER_QUEUE,
                            0, QUEUE_TAG, &tmp_time, qname);

   if (result == DISPATCH_OK) {
      if (violations != nullptr) {
         *violations = compute_soft_violations(a, nullptr, qep, *violations, nullptr, config_attr,
...
sequential_host_time(uint64_t *start, sge_assignment_t *a, int *violations, const lListElem *hep)
{
   ...
   result = rc_time_by_slots(a, hard_requests, load_attr,
         config_attr, actual_attr, hep, nullptr, false,
...
sequential_global_time(uint64_t *start, sge_assignment_t *a, int *violations)
{
   ...
   result = rc_time_by_slots(a, hard_request, load_attr, config_attr, actual_attr, a->gep, nullptr, false, &reason,
                             1, DOMINANT_LAYER_GLOBAL, lc_factor, GLOBAL_TAG, &tmp_time, SGE_GLOBAL_NAME);

   if ((result == DISPATCH_OK) || (result == DISPATCH_MISSING_ATTR)) {
      if (violations != nullptr) {
         *violations = compute_soft_violations(a, nullptr, nullptr, *violations, load_attr, config_attr,
...
parallel_global_slots(sge_assignment_t *a, int *slots)
{
   ...
   if (sge_host_match_static(a, a->gep) == DISPATCH_OK) {
      ...
      result = parallel_rc_slots_by_time(a, &gslots,
                                         config_attr, actual_attr, load_attr, false,
                                         a->gep, nullptr, DOMINANT_LAYER_GLOBAL,
   ...
   *slots      = gslots;
   ...
   if (result == DISPATCH_OK) {
      DPRINTF("\tparallel_global_slots() returns %d\n", gslots);
   } else {
...
parallel_available_slots(const sge_assignment_t *a, int *slots)
{
   ...
   DPRINTF("\tparallel_available_slots(%s) returns %d\n", a->pe_name, *slots);
                 ...
                 int slots, uint32_t layer, double lc_factor, uint64_t *start_time, const char *object_name, ocs::TopologyString& binding_inuse)
{
   ...
   uint32_t relop = lGetUlong(cplx_el, CE_relop);
   lFreeElem(&cplx_el);
                 ...
                 const char *object_name, ocs::TopologyString& binding_inuse)
{
   ...
   int utilized = 0;
   uint32_t consumable = CONSUMABLE_NO;
   ...
   if (exclusive_centry) {
      ...
      DSTRING_STATIC(time_str, 64);
      DPRINTF("\t\t%s: ri_slots_by_time: %s=%f has %d slots at time %s %s (avail: %f total: %f)\n",
              object_name, name, request_val, *slots, sge_ctime64(start, &time_str),
                          ...
                          const char *object_name, bool isRQ)
{
   ...
   for_each_rw (jrs, lGetListRW(a->job, JB_request_set_list)) {
      ...
      if (scope == JRS_SCOPE_MASTER) {
         if (!need_master) {
            ...
            if (queue != nullptr) {
               ...
               print_tagged4schedule(queue);
               if (!lMatchUlongBitMask(queue, QU_tagged4schedule, TAG4SCHED_MASTER)) {
      ...
      if (scope == JRS_SCOPE_SLAVE && queue != nullptr) {
         ...
         print_tagged4schedule(queue);
         if (!lMatchUlongBitMask(queue, QU_tagged4schedule, TAG4SCHED_SLAVE)) {
      ...
      for_each_rw (req, requests) {
         ...
         if (scope == JRS_SCOPE_GLOBAL) {
            // if we get here we have finished handling global requests and have updated max_slots
   ...
   if (a->ar_id != 0) {
      if (a->ar != nullptr) {
         ...
         if (!(a->is_job_verify)) {
            ...
            if (lGetUlong(a->ar, AR_state) == AR_ERROR && lGetUlong(a->ar, AR_error_handling) != 0) {
               schedd_mes_add(a->monitor_alpp, a->monitor_next_run, a->job_id,
                              SCHEDD_INFO_ARISINERROR_I, a->ar_id);
               DRETURN(DISPATCH_NEVER_CAT);
            }

            /* is ar running? */
            if (lGetUlong(a->ar, AR_state) != AR_RUNNING && lGetUlong(a->ar, AR_state) != AR_ERROR) {
               schedd_mes_add(a->monitor_alpp, a->monitor_next_run, a->job_id,
         ...
         /* has user access? */
         if ((acl_list = lGetList(a->ar, AR_xacl_list))) {
         ...
         if ((acl_list = lGetList(a->ar, AR_acl_list))) {
            const lListElem *acl_ep;
            for_each_ep(acl_ep, acl_list) {
               const char *user = lGetString(acl_ep, ARA_name);
...
sge_ar_swap_resource_lists(sge_assignment_t &a) {
   ...
   if (a.ar != nullptr) {
      ...
      lListElem *queue;
      for_each_rw(queue, lGetListRW(a.ar, AR_reserved_queues)) {
...
#if 0
{
   ...
   result = parallel_rc_slots_by_time(a, slots,
                                      ...
                                      false, nullptr, qep, DOMINANT_LAYER_RQS, 0.0, RQS_TAG, need_master, is_master_queue,
