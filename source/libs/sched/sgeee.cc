...
static sge_task_ref_t *task_ref_get_first(uint32_t job_number, uint32_t ja_task_number);
...
static void sgeee_priority(lListElem *task, uint32_t jobid, double nsu, double npri,
...
static uint32_t build_functional_categories(sge_ref_t *job_ref, uint32_t num_jobs, lList **root,
                                        sge_ref_list_t ** ref_array, int dependent, 
                                        uint32_t job_tickets, uint32_t user_tickets, uint32_t prj_tickets, uint32_t dp_tickets);
...
static void task_ref_initialize_table(uint32_t number_of_tasks);
static void task_ref_destroy_table();
static sge_task_ref_t *task_ref_get_entry(uint32_t index);

static scheduler_all_data_t *all_lists; /* thread local */
static uint32_t sge_scheduling_run = 0; /* thread local */

static sge_task_ref_t *task_ref_table = nullptr; /* thread local */
static uint32_t task_ref_entries = 0;   /* thread local */
...
static sge_task_ref_t *task_ref_get_first(uint32_t job_number,
...
static sge_task_ref_t *task_ref_get_next_job_entry()
{
   ...
   sge_task_ref_t *ret = nullptr;
   uint32_t pos = task_ref_job_pos;

   DENTER(BASIS_LAYER); 
   if (current_entry != nullptr) {
      uint32_t current_job_number = current_entry->job_number;
...
void sgeee_resort_pending_jobs(lList **job_list)
{
   ...
   if (next_job) {
      uint32_t job_id = lGetUlong(next_job, JB_job_number);
      uint64_t start_time_of_job1 = lGetUlong64(next_job, JB_submission_time);
      ...
      if (tmp_task == nullptr) {
         ...
         uint32_t ja_task_id = range_list_get_first_id(range_list, nullptr);
      ...
      if (tmp_task) { 
         ...
         for_each_rw (jep, *job_list) {
            uint64_t start_time_of_job2 = lGetUlong64(jep, JB_submission_time);
            uint32_t job_id2 = lGetUlong(jep, JB_job_number);
            const lListElem *tmp_task2 = lFirst(lGetList(jep, JB_ja_tasks));
            double prio2;

            if (tmp_task2 == nullptr) {
...
static void recompute_prio(sge_task_ref_t *tref, lListElem *task, double nurg, double npri)
{
   ...
   DPRINTF("%f tickets for task " sge_u32 ": ntix = %f (min/max %f/%f), prio = %f\n",
           tref->ja_task_ticket, tref->ja_task_number, ntix, min_tix, max_tix, prio);
...
{
   lListElem *root;
   int is_enrolled_ja_task = (ja_task != nullptr && tref == nullptr) ? 1 : 0;
   ...
   ref->ja_task = is_enrolled_ja_task ? ja_task : nullptr;
                                 ...
                                 void *ptr )
{
   ...
   lSetPosList(node, sn_usage_list_pos, nullptr);
...
static void destribute_ftickets(lList *root, int dependent){   
   ...
   for_each_rw_lv(elem, root) {
      first = (sge_ref_list_t *)lGetRef(elem, FCAT_jobrelated_ticket_first);
/*      if(!first)
         continue;*/
      current = first->next;
...
static uint32_t build_functional_categories(sge_ref_t *job_ref, uint32_t num_jobs, lList **fcategories,
   ...
   *ref_array = (sge_ref_list_t *)sge_malloc(sizeof(sge_ref_list_t) * num_jobs);
   ...
   for(job_ndx=0; job_ndx<num_jobs; job_ndx++) {
      ...
      if (jref->queued) { /* only work on pending jobs */
         ...
         uint32_t job_shares = 0;
         uint32_t user_shares = 0;
         uint32_t dept_shares = 0;
         uint32_t project_shares = 0;
         ...
         if (current == nullptr) {   /* create new category */
         ...
         {  /* create new job entry */ 
            sge_ref_list_t  *cur_ref_entry = nullptr;
            sge_ref_list_t *new_elem = &((*ref_array)[ref_array_pos++]);

            new_elem->ref = jref;

            /* find the location for the new element */
            cur_ref_entry = (sge_ref_list_t *)lGetRef(current, FCAT_jobrelated_ticket_last);

            if (dependent) {
               while (cur_ref_entry != nullptr) {
                  ...
                  if (ref->tickets > jref->tickets) {
                     ...
                     uint64_t ref_time = lGetUlong64(ref->job, JB_submission_time);
                     uint64_t jref_time = lGetUlong64(jref->job, JB_submission_time);
                     uint32_t ref_jid = lGetUlong(ref->job, JB_job_number);
                     uint32_t jref_jid = lGetUlong(jref->job, JB_job_number);
            ...
            if (cur_ref_entry == nullptr) {
               if (lGetRef(current, FCAT_jobrelated_ticket_first) == nullptr) {
                  ...
                  cur_ref_entry = (sge_ref_list_t *)lGetRef(current, FCAT_jobrelated_ticket_first);
...
static void free_fcategories(lList **fcategories, sge_ref_list_t **ref_array) {
   ...
   for_each_rw_lv(fcategory, *fcategories) {
      lSetRef(fcategory, FCAT_jobrelated_ticket_last, nullptr);
      lSetRef(fcategory, FCAT_jobrelated_ticket_first, nullptr);
      lSetRef(fcategory, FCAT_user, nullptr);
      lSetRef(fcategory, FCAT_dept, nullptr);
      lSetRef(fcategory, FCAT_project, nullptr);
   }
...
calc_job_tickets ( sge_ref_t *ref )
{
   ...
   if (ja_task && (pe_str=lGetString(ja_task, JAT_granted_pe))
      ...
      nslots = nslots_granted(granted, nullptr);
      active_nslots = active_nslots_granted(job, granted, nullptr);
      ...
      for_each_rw_lv(granted_el, granted) {
         double slots;
...
void sge_clear_job( lListElem *job, bool is_clear_all) {
   if (is_clear_all) {
      ...
      for_each_rw_lv(ja_task, lGetList(job, JB_ja_template)) {
...
sge_clear_ja_task( lListElem *ja_task )
{
   ...
   for_each_rw_lv(granted_el, lGetList(ja_task, JAT_granted_destin_identifier_list)) {
                  ...
                  double *max_tickets)
{
   ...
   uint32_t num_jobs, num_queued_jobs, job_ndx;

   uint32_t num_unenrolled_tasks = 0;

   sge_ref_t *job_ref = nullptr;

   lListElem *job, *root = nullptr;
   ...
   for_each_rw_lv(qep, lists->queue_list) {
   ...
   if (queued_jobs) {
      for_each_rw(job, queued_jobs) {
         uint32_t max = std::min(max_pending_tasks_per_job, free_qslots+1);
         uint32_t num_unenrolled_tasks_for_job = 0;
         uint32_t task_cnt = 0;
   ...
   if (num_jobs > 0) {
      job_ref = (sge_ref_t *)sge_malloc(num_jobs * sizeof(sge_ref_t));
      ...
      for_each_rw(job, running_jobs) {
         for_each_rw_lv(ja_task, lGetList(job, JB_ja_tasks)) {
            if (!job_is_active(job, ja_task)) {
               ...
               sge_set_job_refs(job, ja_task, &job_ref[job_ndx], nullptr, lists, 0);
      ...
      for_each_rw(job, queued_jobs) {
         ...
         for_each_rw(range, range_list) {
            for(id = lGetUlong(range, RN_min);
               ...
               lListElem *ja_task = job_get_ja_task_template_pending(job, id);
   ...
   PROF_STOP_MEASUREMENT(SGE_PROF_SCHEDLIB4);
   prof_pass1= prof_get_measurement_wallclock(SGE_PROF_SCHEDLIB4,false, nullptr);
   PROF_START_MEASUREMENT(SGE_PROF_SCHEDLIB4); 
   ...
   PROF_STOP_MEASUREMENT(SGE_PROF_SCHEDLIB4);
   prof_pass2 = prof_get_measurement_wallclock(SGE_PROF_SCHEDLIB4,false, nullptr);
   PROF_START_MEASUREMENT(SGE_PROF_SCHEDLIB4); 
   ...
   if (queued_jobs) {
      ...
      if (total_share_tree_tickets > 0 && root) {
         ...
         if ((sorted_job_node_list = sge_sort_pending_job_nodes(root, root, total_share_tree_tickets))) {
             ...
             */
            for_each_rw_lv(job_node, sorted_job_node_list) {
      ...
      if (total_functional_tickets > 0 && num_queued_jobs > 0) {
         ...
         uint32_t max =   sconf_get_max_functional_jobs_to_schedule();
         uint32_t pjobs = build_functional_categories(job_ref, num_jobs, &fcategories,
         ...
         if (max > 0) { 
            ...
            for(i=0; i<max; i++) {
               ...
               uint32_t jid, save_jid=0, save_tid=0;
	            uint64_t submission_time = 0, save_submission_time = 0;
               lListElem *max_current = nullptr;

               /* loop over all first jobs from the different categories */
               for_each_rw_lv(current, fcategories) {
                  sge_ref_list_t *tmp = (sge_ref_list_t *)lGetRef(current, FCAT_jobrelated_ticket_first); 
               ...
               if (max_current != nullptr) {
                  ...
                  if (lGetRef(max_current, FCAT_jobrelated_ticket_first) == nullptr) {
                     ...
                     lSetRef(max_current, FCAT_user, nullptr);
                     lSetRef(max_current, FCAT_dept, nullptr);
                     lSetRef(max_current, FCAT_project, nullptr);
                           ...
                           double total_share_tree_tickets)
{
   ...
   if (root != node || job_nodes) { 
      ...
      for_each_rw_lv(job_node, job_node_list) {
         job_count++;
                       ...
                       bool update_usage_and_configuration, int seqno, bool update_execd)
{
   ...
   if (running_jobs) {
      ...
      for_each_rw_lv(job, running_jobs) {
         const char *pe_str = nullptr;
         const lList *granted = nullptr;
 ...
 */
   if ((queued_jobs != nullptr) && (max_queued_ticket_orders || sconf_is_new_config())) {
      uint32_t free_qslots = 0;
      norders = lGetNumberOfElem(order_list);
      for_each_rw_lv(qep, lists->queue_list) {
         free_qslots += MAX(0, ((int)lGetUlong(qep, QU_job_slots) - qinstance_slots_used(qep)));
      ...
      for_each_rw_lv(job, queued_jobs) {
         uint32_t tasks = 0;
         ...
         if (max_queued_ticket_orders) {
            ...
            if (job_get_not_enrolled_ja_tasks(job) > 0) {
               ...
               if (task_template) {
                  order_list = sge_create_orders(order_list, ORT_ptickets, job, task_template, nullptr, false);
               }
            }  
         } else {
            order_list = sge_create_orders(order_list, ORT_clear_pri_info, job, nullptr, nullptr, false);
         }
      }
   ...
   if (update_usage_and_configuration) {
      ...
      if (sconf_is()) {
         ...
         const lDescr schedConfDesc[] = {
                            {SC_weight_tickets_override, lUlongT  | CULL_IS_REDUCED, nullptr},
               ...
               order_t *orders)
{
   ...
   for_each_rw_lv(job, pending_jobs) {
   ...
   {
      uint64_t reprioritize_interval = sge_gmt32_to_gmt64(sconf_get_reprioritize_interval());
               ...
               bool do_nprio, bool do_nurg) 
{
   ...
   for_each_rw_lv (job, job_list) {
      ...
      for_each_rw_lv (task, lGetList(job, JB_ja_tasks)) {
         sgeee_priority(task, jobid, nsu, npri, min_tix, max_tix);
         enrolled = true;
      }

      if (!enrolled) {
         lListElem *task = lFirstRW(lGetList(job, JB_ja_template));
         sgeee_priority(task, jobid, nsu, npri, min_tix, max_tix);
      }
   }
}
...
static void calculate_pending_shared_override_tickets(sge_ref_t *job_ref, uint32_t num_jobs, int dependent) {
         ...
         if (max > 0) {
            ...
            for (i = 0; i < max; i++) {        
               ...
               uint32_t jid, tid, max_jid=0, max_tid=0;
...
main(int argc, char **argv)
{
   ...
   sge_Sdescr_t *lists;
   lList *child_list = nullptr;
   lList *group_list = nullptr;
   lListElem *job, *config, *node;

   lListElem *ep;

   lists = (sge_Sdescr_t *)sge_malloc(sizeof(sge_Sdescr_t));
   ...
   lSetList(config, SC_usage_weight_list,
       build_usage_list("usageweightlist", nullptr));
   for_each_ep_lv(usage, lGetList(config, SC_usage_weight_list))
   ...
   for_each_ep_lv(usage, lGetList(job, JB_scaled_usage_list))
   ...
   sge_setup_lists(lists, lists->job_list, nullptr);

   sge_calc_tickets(lists, lists->job_list, nullptr, nullptr);
