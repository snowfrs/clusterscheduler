      ...
      case SP_DEST_TMP:
         {
             ...
             * thread safe.
             */
            filepath_in = sge_tmpnam(buffer, &fd, &tmp_name_error);
            if (filepath_in == nullptr) {
               if (sge_dstring_get_string(&tmp_name_error) != nullptr) {
                  answer_list_add_sprintf(answer_list, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR, sge_dstring_get_string(&tmp_name_error));
               } else {
                  answer_list_add_sprintf(answer_list, STATUS_EUNKNOWN, ANSWER_QUALITY_ERROR, MSG_ERRORGETTINGTMPNAM_S, strerror(errno));
               }
                            ...
                            bool parse_values)
{
   ...
   if (fields_out != nullptr) {
      ...
      my_fields_out = (int *)sge_malloc ((spool_get_number_of_fields (fields) + 1) *
                                                                  sizeof (int));
...
int spool_get_unprocessed_field(spooling_field in[], int out[], lList **alpp)
{
   ...
   while (in[count].nm != NoName) {
      ...
      if (out[counter] == NoName) {
         snprintf(SGE_EVENT, SGE_EVENT_SIZE, MSG_FLATFILE_ATTRIBISMISSING_S, (in[count].name == nullptr) ? lNm2Str(in[count].nm) : in[count].name);
...
static void spool_flatfile_add_line_breaks(dstring *buffer)
{
   ...
   while (strlen(strp) > MAX_LINE_LENGTH - word) {
      ...
      if (indent_str == nullptr) {
         ...
         indent_str = sge_malloc((word + 4) * sizeof (char));
         indent_str[0] = ' ';
         indent_str[1] = '\\';
         indent_str[2] = '\n';
