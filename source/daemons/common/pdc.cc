...
#if defined(FREEBSD) || defined(LINUX) || defined(SOLARIS) || defined(DARWIN)
      {
         //int proccount;
...
struct psJob_s *psGetOneJob(JobID_t JobID)
{
   ...
   if (found) {
      ...
      if ((rjob = (struct rjob_s *)sge_malloc(rsize))) {
...
*/
   ...
   if (sgeview) {
      ...
      curr_cpu = (double *)sge_malloc(numjobs * sizeof(double));
      memset(curr_cpu, 0, numjobs*sizeof(double));
      prev_cpu = (double *)sge_malloc(numjobs * sizeof(double));
      memset(prev_cpu, 0, numjobs*sizeof(double));
      diff_cpu = (double *)sge_malloc(numjobs * sizeof(double));
