...
static int wait_for_qrsh_socket(int sock, int timeout)
{
   ...
   switch(select(sock + 1, &ready, (fd_set *)0, (fd_set *)0, &to)) {
      case -1:
         ERROR(MSG_QSH_ERRORWAITINGONSOCKETFORCLIENTTOCONNECT_S, strerror(errno));
         break;
      case 0:
      ...
      default:
         if (FD_ISSET(sock, &ready)) {
            ...
            msgsock = accept(sock,(struct sockaddr *) 0,(socklen_t *) nullptr);
   ...
   do {
      *c = 0;
      ...
      if ((rval = read(msgsock, c, 1)) == -1) {
         ERROR(MSG_QSH_ERRORREADINGSTREAMMESSAGE_S, strerror(errno));
         DRETURN(nullptr);
      } else {
         if (rval == 0) {
            ERROR(SFNMAX, MSG_QSH_ERRORENDINGCONNECTION);
            DRETURN(nullptr);
         }
      }
   } while (*c++ != 0);
...
static int get_remote_exit_code(int sock)
{
   ...
   ERROR(SFNMAX, MSG_QSH_ERRORREADINGRETURNCODEOFREMOTECOMMAND);
   DRETURN(-1);
}
   ...
   for_each_ep(aep, alp) {
      ...
      if (quality == ANSWER_QUALITY_ERROR) {
         ...
         *alp_error = 1;
         ...
         WARNING(SFN SFN, MSG_WARNING, lGetString(aep, AN_text));
      }
   }
                                ...
                                int sock)
{
   ...
   if (child_pid == -1) {
      ERROR(MSG_QSH_CANNOTFORKPROCESS_S, strerror(errno));
      DRETURN(1);
   }
   ...
   if (child_pid) {
      ...
      sigaction(SIGINT, &forward_action, nullptr);
      sigaction(SIGQUIT, &forward_action, nullptr);
      sigaction(SIGTERM, &forward_action, nullptr);
      ...
      if (ret != 0) {
         ERROR(MSG_QSH_UNMATCHED_C, ret==1 ? '\"' : '\'');
         exit(EXIT_FAILURE);
      }
      ...
      if (is_rsh || is_rlogin) {
         ...
         if (is_rsh && nostdin) {
            sge_sl_insert(sl_args, (void *)"-n", SGE_SL_BACKWARD);
         }

         sge_sl_insert(sl_args, (void *)"-p", SGE_SL_BACKWARD);
         ...
         if (is_rsh) {
            ...
            if (noshell) {
               sge_sl_insert(sl_args, (void *)"noshell", SGE_SL_BACKWARD);
            }
         }
      } else {
         sge_sl_insert(sl_args, (void*)host, SGE_SL_BACKWARD);
      ...
      execvp(args[0], args);
      ERROR(MSG_EXEC_CANTEXECXYZ_SS, args[0], strerror(errno));
      exit(EXIT_FAILURE);
   }
...
static int get_client_server_context(int msgsock, char **port, char **job_dir, char **utilbin_dir, const char **host)
{
   ...
   if (data == nullptr || *data == 0) {
      ERROR(MSG_QSH_ERRORREADINGCONTEXTFROMQLOGIN_STARTER_S,"qlogin_starter");
      DRETURN(0);
   }
   ...
   if (strcmp(s_code, "0") == 0) {  /* qlogin_starter reports valid data */
      if ((*port = strtok(nullptr, ":")) != nullptr) {
         if ((*utilbin_dir = strtok(nullptr, ":")) != nullptr) {
            if ((*job_dir = strtok(nullptr, ":")) != nullptr) {
               if ((*host = strtok(nullptr, ":")) != nullptr) {
                  return 1;
               }
            }
         }
      ...
      ERROR("%s: %s\n", s_code, message == nullptr ? "" : message);
   }
...
get_client_name(int is_rsh, int is_rlogin, int inherit_job)
{
   ...
   const lListElem *qlogin_cmd_elem = nullptr;
    ...
    */
   if (client_name == nullptr || sge_strnullcasecmp(client_name, "none") == 0) {
      if (is_rsh || is_rlogin) {
      ...
      */
      ...
      if (ret != 0) {
         if (ret == 1) {
            ...
            ERROR(MSG_QSH_SETTINGCONNECTIONPARAMS_S, sge_dstring_get_string(&err_msg));
         }
         sge_exit(1);
      }
   ...
   if (existing_job) {
      ...
      if (!g_new_interactive_job_support) {
         ...
         if (ret != COMM_RETVAL_OK) {
            ERROR(MSG_QSH_GOTNOCONNECTIONWITHINSECONDS_IS, QSH_SOCKET_FINAL_TIMEOUT, sge_dstring_get_string(&err_msg));
      ...
      while (!do_exit) {
         ...
         if (do_shut || do_exit) {
            WARNING(SFNMAX, MSG_QSH_REQUESTFORINTERACTIVEJOBHASBEENCANCELED);
            delete_job(job_id, lp_jobs);
            lFreeList(&lp_poll);
            do_exit = 1;
            exit_status = 1;
         ...
         if ((lp_poll == nullptr || lGetNumberOfElem(lp_poll) == 0 ) || !(jep = lFirstRW(lp_poll))) {
            WARNING("\n");
            log_state_set_log_verbose(1);
            WARNING(MSG_QSH_REQUESTCANTBESCHEDULEDTRYLATER_S, progname);
            do_exit = 1;
            exit_status = 1;
            lFreeList(&lp_poll);
            continue;
      ...
      if (g_new_interactive_job_support && comm_handle != nullptr) {
         ...
         if (force_ijs_server_shutdown(&comm_handle, COMM_CLIENT, &err_msg) != 0) {
            ERROR("%s", sge_dstring_get_string(&err_msg));
         }
         sge_dstring_free(&err_msg);
      }
                                ...
                                int is_qlogin, int is_rsh, int is_qsh)
{
   ...
   while ((ep = next) != nullptr) {
      ...
      if (cp != nullptr) {
         ...
         // the login type interactive jobs do not allow some options - delete these
         if (JOB_TYPE_IS_QLOGIN(jb_now) || JOB_TYPE_IS_QRLOGIN(jb_now)) {
            ...
            // some options are only available when the transport method is "builtin" (new interactive job support)
         ...
         /* qrsh -inherit only allows -cwd and setting of environment */
